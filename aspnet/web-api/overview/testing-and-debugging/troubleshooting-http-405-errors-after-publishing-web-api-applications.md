...
---
# <a name="troubleshooting-http-405-errors-after-publishing-web-api-applications"></a>Solución de problemas de errores HTTP 405 después de publicar aplicaciones de API Web

> En este tutorial se describe cómo solucionar los errores HTTP 405 después de publicar una aplicación de API Web en un servidor Web de producción.
> 
...
Las aplicaciones de API Web suelen usar varios verbos HTTP comunes: GET, POST, PUT, DELETE y algunas veces PATCH. Dicho esto, los desarrolladores pueden encontrarse en situaciones en las que otro módulo de IIS implementa esos verbos en el servidor de producción, lo que conduce a una situación en la que un controlador de API Web que funciona correctamente en Visual Studio o en un servidor de desarrollo devolverá un HTTP 405 error cuando se implementa en un servidor de producción. Afortunadamente, este problema se resuelve fácilmente, pero la resolución garantiza una explicación de por qué se está produciendo el problema.
...
Este escenario se suele encontrar después de publicar una aplicación desde un entorno de desarrollo a un entorno de producción, lo que se debe a que la lista de controladores o módulos es diferente entre los entornos de desarrollo y producción. Por ejemplo, si usa Visual Studio 2012 o una versión posterior para desarrollar una aplicación de API Web, IIS Express es el servidor Web predeterminado para las pruebas. Este servidor Web de desarrollo es una versión reducida de la funcionalidad de IIS completa que se incluye en un producto de servidor, y este servidor Web de desarrollo contiene algunos cambios que se agregaron para los escenarios de desarrollo. Por ejemplo, el módulo WebDAV suele instalarse en un servidor Web de producción que ejecuta la versión completa de IIS, aunque es posible que no esté en uso real. La versión de desarrollo de IIS (IIS Express) instala el módulo WebDAV, pero las entradas del módulo WebDAV se marcan como comentario intencionadamente, por lo que el módulo WebDAV nunca se carga en IIS Express a menos que se modifique específicamente la configuración de IIS Express configuración para agregar funcionalidad de WebDAV a la instalación de IIS Express. Como resultado, la aplicación web puede funcionar correctamente en el equipo de desarrollo, pero es posible que se produzcan errores HTTP 405 al publicar la aplicación de la API Web en el servidor Web de producción.
...
Si se produce una situación en la que recibe un mensaje de error HTTP 501, lo que significa que la funcionalidad específica no se ha implementado en el servidor, esto suele significar que no hay ningún controlador definido en la configuración de IIS que coincida con la solicitud HTTP, que probablemente indica que algo no se instaló correctamente en el sistema, o que ha modificado la configuración de IIS para que no haya ningún controlador definido que admita el método HTTP específico. Para resolver ese problema, debe volver a instalar cualquier aplicación que esté intentando usar un método HTTP para el que no tiene definiciones de módulo o de controlador correspondientes.

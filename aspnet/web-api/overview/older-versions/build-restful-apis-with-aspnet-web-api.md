...
1. <span data-ttu-id="52e8f-158">Eliminar el archivo denominado **ValuesController.cs** en **controladores** carpeta del proyecto.</span><span class="sxs-lookup"><span data-stu-id="52e8f-158">Delete the file named **ValuesController.cs** within **Controllers** folder from the project.</span></span>
...
5. <span data-ttu-id="52e8f-169">Presione **F5** para depurar la aplicación.</span><span class="sxs-lookup"><span data-stu-id="52e8f-169">Press **F5** to debug the application.</span></span> <span data-ttu-id="52e8f-170">Debería aparecer la página principal predeterminada para un proyecto Web API.</span><span class="sxs-lookup"><span data-stu-id="52e8f-170">The default home page for a Web API project should appear.</span></span>
    ...
    <span data-ttu-id="52e8f-172">*La página principal predeterminada de una aplicación de ASP.NET Web API*</span><span class="sxs-lookup"><span data-stu-id="52e8f-172">*The default home page of an ASP.NET Web API application*</span></span>
...
2. <span data-ttu-id="52e8f-197">En el **Agregar nuevo elemento** cuadro de diálogo, el nombre del nuevo archivo **Contact.cs** y haga clic en **agregar.**</span><span class="sxs-lookup"><span data-stu-id="52e8f-197">In the **Add New Item** dialog, name the new file **Contact.cs** and click **Add.**</span></span>
    ...
    <span data-ttu-id="52e8f-199">*Crear el nuevo archivo de clase de contacto*</span><span class="sxs-lookup"><span data-stu-id="52e8f-199">*Creating the new Contact class file*</span></span>
...
3. <span data-ttu-id="52e8f-280">Modifique el código para el **GetAllContacts** método tal y como se muestra a continuación.</span><span class="sxs-lookup"><span data-stu-id="52e8f-280">Modify the code for the **GetAllContacts** method as demonstrated below.</span></span>
    ...
    <span data-ttu-id="52e8f-281">(Código de fragmento de código - *Web API laboratorio - Ex02 - obtener todos los contactos*)</span><span class="sxs-lookup"><span data-stu-id="52e8f-281">(Code Snippet - *Web API Lab - Ex02 - Get All Contacts*)</span></span>
...
3. <span data-ttu-id="52e8f-374">Seleccione el fragmento de código relevante en la lista, haciendo clic en él.</span><span class="sxs-lookup"><span data-stu-id="52e8f-374">Pick the relevant snippet from the list, by clicking on it.</span></span>
    ...
    <span data-ttu-id="52e8f-376">*Haga doble clic donde desea insertar el fragmento de código y seleccione Insertar fragmento de código*</span><span class="sxs-lookup"><span data-stu-id="52e8f-376">*Right-click where you want to insert the code snippet and select Insert Snippet*</span></span>
...
<span data-ttu-id="52e8f-403">En este apéndice se mostrará cómo crear un nuevo sitio web desde el Portal de Azure y publicar la aplicación que obtuvo siguiendo el laboratorio, que aprovecha la característica de publicación Web Deploy proporcionada por Azure.</span><span class="sxs-lookup"><span data-stu-id="52e8f-403">This appendix will show you how to create a new web site from the Azure Portal and publish the application you obtained by following the lab, taking advantage of the Web Deploy publishing feature provided by Azure.</span></span>

<a id="ApxCTask1"></a>

<a id="Task_1_-_Creating_a_New_Web_Site_from_the_Windows_Azure_Portal"></a>
#### <a name="task-1---creating-a-new-web-site-from-the-azure-portal"></a><span data-ttu-id="52e8f-404">Tarea 1: crear un nuevo sitio Web desde el Portal de Azure</span><span class="sxs-lookup"><span data-stu-id="52e8f-404">Task 1 - Creating a New Web Site from the Azure Portal</span></span>

1. <span data-ttu-id="52e8f-405">Vaya a la [Portal de administración de Azure](https://manage.windowsazure.com/) e inicie sesión con las credenciales de Microsoft asociadas con su suscripción.</span><span class="sxs-lookup"><span data-stu-id="52e8f-405">Go to the [Azure Management Portal](https://manage.windowsazure.com/) and sign in using the Microsoft credentials associated with your subscription.</span></span>

    > [!NOTE]
    > <span data-ttu-id="52e8f-406">Con Azure puede hospedar 10 sitios Web ASP.NET de forma gratuita y, a continuación, escalar a medida que crece el tráfico.</span><span class="sxs-lookup"><span data-stu-id="52e8f-406">With Azure you can host 10 ASP.NET Web Sites for free and then scale as your traffic grows.</span></span> <span data-ttu-id="52e8f-407">Puede registrarse [aquí](https://aka.ms/aspnet-hol-azure).</span><span class="sxs-lookup"><span data-stu-id="52e8f-407">You can sign up [here](https://aka.ms/aspnet-hol-azure).</span></span>

    <span data-ttu-id="52e8f-408">![Inicie sesión en el portal de Windows Azure](build-restful-apis-with-aspnet-web-api/_static/image39.png "inicie sesión en el portal de Windows Azure")</span><span class="sxs-lookup"><span data-stu-id="52e8f-408">![Log on to Windows Azure portal](build-restful-apis-with-aspnet-web-api/_static/image39.png "Log on to Windows Azure portal")</span></span>

    <span data-ttu-id="52e8f-409">*Inicie sesión en el Portal*</span><span class="sxs-lookup"><span data-stu-id="52e8f-409">*Log on to Portal*</span></span>
2. <span data-ttu-id="52e8f-410">Haga clic en **New** en la barra de comandos.</span><span class="sxs-lookup"><span data-stu-id="52e8f-410">Click **New** on the command bar.</span></span>

    <span data-ttu-id="52e8f-411">![Crear un nuevo sitio Web](build-restful-apis-with-aspnet-web-api/_static/image40.png "crear un nuevo sitio Web")</span><span class="sxs-lookup"><span data-stu-id="52e8f-411">![Creating a new Web Site](build-restful-apis-with-aspnet-web-api/_static/image40.png "Creating a new Web Site")</span></span>

    <span data-ttu-id="52e8f-412">*Crear un nuevo sitio Web*</span><span class="sxs-lookup"><span data-stu-id="52e8f-412">*Creating a new Web Site*</span></span>
3. <span data-ttu-id="52e8f-413">Haga clic en **proceso** | **sitio Web**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-413">Click **Compute** | **Web Site**.</span></span> <span data-ttu-id="52e8f-414">A continuación, seleccione **creación rápida** opción.</span><span class="sxs-lookup"><span data-stu-id="52e8f-414">Then select **Quick Create** option.</span></span> <span data-ttu-id="52e8f-415">Proporcione una dirección URL disponible para el nuevo sitio web y haga clic en **crear sitio Web**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-415">Provide an available URL for the new web site and click **Create Web Site**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="52e8f-416">Azure es el host para una aplicación web que se ejecutan en la nube que puede controlar y administrar.</span><span class="sxs-lookup"><span data-stu-id="52e8f-416">Azure is the host for a web application running in the cloud that you can control and manage.</span></span> <span data-ttu-id="52e8f-417">La opción Creación rápida permite implementar una aplicación web completada en Azure desde fuera del portal.</span><span class="sxs-lookup"><span data-stu-id="52e8f-417">The Quick Create option allows you to deploy a completed web application to the Azure from outside the portal.</span></span> <span data-ttu-id="52e8f-418">No incluye los pasos para configurar una base de datos.</span><span class="sxs-lookup"><span data-stu-id="52e8f-418">It does not include steps for setting up a database.</span></span>

    <span data-ttu-id="52e8f-419">![Crear un nuevo sitio Web mediante Creación rápida](build-restful-apis-with-aspnet-web-api/_static/image41.png "crear un nuevo sitio Web mediante Creación rápida")</span><span class="sxs-lookup"><span data-stu-id="52e8f-419">![Creating a new Web Site using Quick Create](build-restful-apis-with-aspnet-web-api/_static/image41.png "Creating a new Web Site using Quick Create")</span></span>

    <span data-ttu-id="52e8f-420">*Crear un nuevo sitio Web mediante Creación rápida*</span><span class="sxs-lookup"><span data-stu-id="52e8f-420">*Creating a new Web Site using Quick Create*</span></span>
4. <span data-ttu-id="52e8f-421">Espere hasta que el nuevo **sitio Web** se crea.</span><span class="sxs-lookup"><span data-stu-id="52e8f-421">Wait until the new **Web Site** is created.</span></span>
5. <span data-ttu-id="52e8f-422">Una vez creado el sitio Web, haga clic en el vínculo situado bajo el **URL** columna.</span><span class="sxs-lookup"><span data-stu-id="52e8f-422">Once the Web Site is created click the link under the **URL** column.</span></span> <span data-ttu-id="52e8f-423">Compruebe que funciona el nuevo sitio Web.</span><span class="sxs-lookup"><span data-stu-id="52e8f-423">Check that the new Web Site is working.</span></span>

    <span data-ttu-id="52e8f-424">![Exploración al sitio web nuevo](build-restful-apis-with-aspnet-web-api/_static/image42.png "exploración al sitio web nuevo")</span><span class="sxs-lookup"><span data-stu-id="52e8f-424">![Browsing to the new web site](build-restful-apis-with-aspnet-web-api/_static/image42.png "Browsing to the new web site")</span></span>

    <span data-ttu-id="52e8f-425">*Exploración al sitio web nuevo*</span><span class="sxs-lookup"><span data-stu-id="52e8f-425">*Browsing to the new web site*</span></span>

    <span data-ttu-id="52e8f-426">![Sitio Web que se ejecuta](build-restful-apis-with-aspnet-web-api/_static/image43.png "sitio Web que se ejecuta")</span><span class="sxs-lookup"><span data-stu-id="52e8f-426">![Web site running](build-restful-apis-with-aspnet-web-api/_static/image43.png "Web site running")</span></span>

    <span data-ttu-id="52e8f-427">*Sitio Web que se ejecuta*</span><span class="sxs-lookup"><span data-stu-id="52e8f-427">*Web site running*</span></span>
6. <span data-ttu-id="52e8f-428">Vuelva al portal y haga clic en el nombre del sitio web en el **nombre** columna para mostrar las páginas de administración.</span><span class="sxs-lookup"><span data-stu-id="52e8f-428">Go back to the portal and click the name of the web site under the **Name** column to display the management pages.</span></span>

    <span data-ttu-id="52e8f-429">![Abrir las páginas de administración del sitio web](build-restful-apis-with-aspnet-web-api/_static/image44.png "abrir las páginas de administración del sitio web")</span><span class="sxs-lookup"><span data-stu-id="52e8f-429">![Opening the web site management pages](build-restful-apis-with-aspnet-web-api/_static/image44.png "Opening the web site management pages")</span></span>

    <span data-ttu-id="52e8f-430">*Abrir las páginas de administración del sitio Web*</span><span class="sxs-lookup"><span data-stu-id="52e8f-430">*Opening the Web Site management pages*</span></span>
7. <span data-ttu-id="52e8f-431">En el **panel** página, en el **primea** sección, haga clic en el **descargar perfil de publicación** vínculo.</span><span class="sxs-lookup"><span data-stu-id="52e8f-431">In the **Dashboard** page, under the **quick glance** section, click the **Download publish profile** link.</span></span>

    > [!NOTE]
    > <span data-ttu-id="52e8f-432">El *perfil de publicación* contiene toda la información necesaria para publicar una aplicación web en un Azure para cada método de publicación habilitado.</span><span class="sxs-lookup"><span data-stu-id="52e8f-432">The *publish profile* contains all of the information required to publish a web application to a Azure for each enabled publication method.</span></span> <span data-ttu-id="52e8f-433">El perfil de publicación contiene las direcciones URL, las credenciales de usuario y las cadenas de base de datos necesarias para conectarse y autenticarse en cada uno de los puntos de conexión para el que está habilitado un método de publicación.</span><span class="sxs-lookup"><span data-stu-id="52e8f-433">The publish profile contains the URLs, user credentials and database strings required to connect to and authenticate against each of the endpoints for which a publication method is enabled.</span></span> <span data-ttu-id="52e8f-434">**Microsoft WebMatrix 2**, **Microsoft Visual Studio Express para Web** y **Microsoft Visual Studio 2012** admiten la lectura de perfiles de publicación para automatizar la configuración de estos programas para publicación de aplicaciones web en Azure.</span><span class="sxs-lookup"><span data-stu-id="52e8f-434">**Microsoft WebMatrix 2**, **Microsoft Visual Studio Express for Web** and **Microsoft Visual Studio 2012** support reading publish profiles to automate configuration of these programs for publishing web applications to Azure.</span></span>

    <span data-ttu-id="52e8f-435">![Descargando el sitio web de perfil de publicación](build-restful-apis-with-aspnet-web-api/_static/image45.png "descargando el sitio web de perfil de publicación")</span><span class="sxs-lookup"><span data-stu-id="52e8f-435">![Downloading the web site publish profile](build-restful-apis-with-aspnet-web-api/_static/image45.png "Downloading the web site publish profile")</span></span>

    <span data-ttu-id="52e8f-436">*Descargando el sitio Web de perfil de publicación*</span><span class="sxs-lookup"><span data-stu-id="52e8f-436">*Downloading the Web Site publish profile*</span></span>
8. <span data-ttu-id="52e8f-437">Descargue el archivo de perfil de publicación en una ubicación conocida.</span><span class="sxs-lookup"><span data-stu-id="52e8f-437">Download the publish profile file to a known location.</span></span> <span data-ttu-id="52e8f-438">Aún más en este ejercicio verá cómo usar este archivo para publicar una aplicación web en Azure desde Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="52e8f-438">Further in this exercise you will see how to use this file to publish a web application to Azure from Visual Studio.</span></span>

    <span data-ttu-id="52e8f-439">![Guardar el archivo de perfil de publicación](build-restful-apis-with-aspnet-web-api/_static/image46.png "guardar el perfil de publicación")</span><span class="sxs-lookup"><span data-stu-id="52e8f-439">![Saving the publish profile file](build-restful-apis-with-aspnet-web-api/_static/image46.png "Saving the publish profile")</span></span>

    <span data-ttu-id="52e8f-440">*Guardar el archivo de perfil de publicación*</span><span class="sxs-lookup"><span data-stu-id="52e8f-440">*Saving the publish profile file*</span></span>

<a id="ApxCTask2"></a>

<a id="Task_2_-_Configuring_the_Database_Server"></a>
#### <a name="task-2---configuring-the-database-server"></a><span data-ttu-id="52e8f-441">Tarea 2: configurar el servidor de base de datos</span><span class="sxs-lookup"><span data-stu-id="52e8f-441">Task 2 - Configuring the Database Server</span></span>

<span data-ttu-id="52e8f-442">Si la aplicación hace uso de SQL Server deberá crear un servidor de base de datos SQL de bases de datos.</span><span class="sxs-lookup"><span data-stu-id="52e8f-442">If your application makes use of SQL Server databases you will need to create a SQL Database server.</span></span> <span data-ttu-id="52e8f-443">Si desea implementar una aplicación simple que no utiliza SQL Server, podría omitir esta tarea.</span><span class="sxs-lookup"><span data-stu-id="52e8f-443">If you want to deploy a simple application that does not use SQL Server you might skip this task.</span></span>

1. <span data-ttu-id="52e8f-444">Necesitará un servidor de base de datos SQL para almacenar la base de datos de aplicación.</span><span class="sxs-lookup"><span data-stu-id="52e8f-444">You will need a SQL Database server for storing the application database.</span></span> <span data-ttu-id="52e8f-445">Puede ver los servidores de base de datos SQL de la suscripción en el portal de administración de Azure en **bases de datos Sql** | **servidores** | **panel del servidor**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-445">You can view the SQL Database servers from your subscription in the Azure Management portal at **Sql Databases** | **Servers** | **Server's Dashboard**.</span></span> <span data-ttu-id="52e8f-446">Si no tiene un servidor creado, puede crear una mediante el **agregar** botón en la barra de comandos.</span><span class="sxs-lookup"><span data-stu-id="52e8f-446">If you do not have a server created, you can create one using the **Add** button on the command bar.</span></span> <span data-ttu-id="52e8f-447">Tome nota de la **nombre del servidor y dirección URL, nombre de inicio de sesión de administrador y contraseña**, ya que lo usará en las siguientes tareas.</span><span class="sxs-lookup"><span data-stu-id="52e8f-447">Take note of the **server name and URL, administrator login name and password**, as you will use them in the next tasks.</span></span> <span data-ttu-id="52e8f-448">No cree la base de datos, como se crearán en una etapa posterior.</span><span class="sxs-lookup"><span data-stu-id="52e8f-448">Do not create the database yet, as it will be created in a later stage.</span></span>

    <span data-ttu-id="52e8f-449">![Panel de base de datos SQL Server](build-restful-apis-with-aspnet-web-api/_static/image47.png "panel de base de datos SQL Server")</span><span class="sxs-lookup"><span data-stu-id="52e8f-449">![SQL Database Server Dashboard](build-restful-apis-with-aspnet-web-api/_static/image47.png "SQL Database Server Dashboard")</span></span>

    <span data-ttu-id="52e8f-450">*Panel de base de datos SQL Server*</span><span class="sxs-lookup"><span data-stu-id="52e8f-450">*SQL Database Server Dashboard*</span></span>
2. <span data-ttu-id="52e8f-451">En la siguiente tarea probará la conexión de base de datos desde Visual Studio, por ese motivo debe incluir la dirección IP local en la lista del servidor de **direcciones IP permitidas**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-451">In the next task you will test the database connection from Visual Studio, for that reason you need to include your local IP address in the server's list of **Allowed IP Addresses**.</span></span> <span data-ttu-id="52e8f-452">Para ello, haga clic en **configurar**, seleccione la dirección IP de **dirección IP del cliente actual** y péguelo en el **dirección IP inicial** y **ladirecciónIPfinal** cuadros de texto y haga clic en el ![add-client-ip-address-ok-button](build-restful-apis-with-aspnet-web-api/_static/image48.png) botón.</span><span class="sxs-lookup"><span data-stu-id="52e8f-452">To do that, click **Configure**, select the IP address from **Current Client IP Address** and paste it on the **Start IP Address** and **End IP Address** text boxes and click the ![add-client-ip-address-ok-button](build-restful-apis-with-aspnet-web-api/_static/image48.png) button.</span></span>

    ![Agregar dirección IP del cliente](build-restful-apis-with-aspnet-web-api/_static/image49.png)

    <span data-ttu-id="52e8f-454">*Agregar dirección IP del cliente*</span><span class="sxs-lookup"><span data-stu-id="52e8f-454">*Adding Client IP Address*</span></span>
3. <span data-ttu-id="52e8f-455">Una vez el **dirección IP del cliente** se agrega a las direcciones IP permitidas de lista, haga clic en **guardar** para confirmar los cambios.</span><span class="sxs-lookup"><span data-stu-id="52e8f-455">Once the **Client IP Address** is added to the allowed IP addresses list, click on **Save** to confirm the changes.</span></span>

    ![Confirmar cambios](build-restful-apis-with-aspnet-web-api/_static/image50.png)

    <span data-ttu-id="52e8f-457">*Confirmar cambios*</span><span class="sxs-lookup"><span data-stu-id="52e8f-457">*Confirm Changes*</span></span>

<a id="ApxCTask3"></a>

<a id="Task_3_-_Publishing_an_ASPNET_MVC_4_Application_using_Web_Deploy"></a>
#### <a name="task-3---publishing-an-aspnet-mvc-4-application-using-web-deploy"></a><span data-ttu-id="52e8f-458">Tarea 3: publicar una aplicación de ASP.NET MVC 4 mediante Web Deploy</span><span class="sxs-lookup"><span data-stu-id="52e8f-458">Task 3 - Publishing an ASP.NET MVC 4 Application using Web Deploy</span></span>

1. <span data-ttu-id="52e8f-459">Vuelva a la solución de ASP.NET MVC 4.</span><span class="sxs-lookup"><span data-stu-id="52e8f-459">Go back to the ASP.NET MVC 4 solution.</span></span> <span data-ttu-id="52e8f-460">En el **el Explorador de soluciones**, haga clic en el proyecto de sitio web y seleccione **publicar**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-460">In the **Solution Explorer**, right-click the web site project and select **Publish**.</span></span>

    <span data-ttu-id="52e8f-461">![Publicar la aplicación](build-restful-apis-with-aspnet-web-api/_static/image51.png "publicar la aplicación")</span><span class="sxs-lookup"><span data-stu-id="52e8f-461">![Publishing the Application](build-restful-apis-with-aspnet-web-api/_static/image51.png "Publishing the Application")</span></span>

    <span data-ttu-id="52e8f-462">*Publicar el sitio web*</span><span class="sxs-lookup"><span data-stu-id="52e8f-462">*Publishing the web site*</span></span>
2. <span data-ttu-id="52e8f-463">Importar el perfil de publicación que guardó en la primera tarea.</span><span class="sxs-lookup"><span data-stu-id="52e8f-463">Import the publish profile you saved in the first task.</span></span>

    <span data-ttu-id="52e8f-464">![Importar el perfil de publicación](build-restful-apis-with-aspnet-web-api/_static/image52.png "importar el perfil de publicación")</span><span class="sxs-lookup"><span data-stu-id="52e8f-464">![Importing the publish profile](build-restful-apis-with-aspnet-web-api/_static/image52.png "Importing the publish profile")</span></span>

    <span data-ttu-id="52e8f-465">*Importar perfil de publicación*</span><span class="sxs-lookup"><span data-stu-id="52e8f-465">*Importing publish profile*</span></span>
3. <span data-ttu-id="52e8f-466">Haga clic en **validar conexión**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-466">Click **Validate Connection**.</span></span> <span data-ttu-id="52e8f-467">Una vez completada la validación, haga clic en **siguiente**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-467">Once Validation is complete click **Next**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="52e8f-468">Validación está completa cuando aparece una marca de verificación verde junto al botón Validar conexión.</span><span class="sxs-lookup"><span data-stu-id="52e8f-468">Validation is complete once you see a green checkmark appear next to the Validate Connection button.</span></span>

    <span data-ttu-id="52e8f-469">![Validación de la conexión](build-restful-apis-with-aspnet-web-api/_static/image53.png "validación de la conexión")</span><span class="sxs-lookup"><span data-stu-id="52e8f-469">![Validating connection](build-restful-apis-with-aspnet-web-api/_static/image53.png "Validating connection")</span></span>

    <span data-ttu-id="52e8f-470">*Validación de la conexión*</span><span class="sxs-lookup"><span data-stu-id="52e8f-470">*Validating connection*</span></span>
4. <span data-ttu-id="52e8f-471">En el **configuración** página, en el **bases de datos** sección, haga clic en el botón situado junto al cuadro de texto de la conexión de base de datos (es decir, **DefaultConnection**).</span><span class="sxs-lookup"><span data-stu-id="52e8f-471">In the **Settings** page, under the **Databases** section, click the button next to your database connection's textbox (i.e. **DefaultConnection**).</span></span>

    <span data-ttu-id="52e8f-472">![Configuración de implementación Web](build-restful-apis-with-aspnet-web-api/_static/image54.png "configuración de implementación Web")</span><span class="sxs-lookup"><span data-stu-id="52e8f-472">![Web deploy configuration](build-restful-apis-with-aspnet-web-api/_static/image54.png "Web deploy configuration")</span></span>

    <span data-ttu-id="52e8f-473">*Configuración de implementación Web*</span><span class="sxs-lookup"><span data-stu-id="52e8f-473">*Web deploy configuration*</span></span>
5. <span data-ttu-id="52e8f-474">Configure la conexión de base de datos como sigue:</span><span class="sxs-lookup"><span data-stu-id="52e8f-474">Configure the database connection as follows:</span></span>

   - <span data-ttu-id="52e8f-475">En el **nombre del servidor** escriba la dirección URL de base de datos de SQL server mediante el *tcp:* prefijo.</span><span class="sxs-lookup"><span data-stu-id="52e8f-475">In the **Server name** type your SQL Database server URL using the *tcp:* prefix.</span></span>
   - <span data-ttu-id="52e8f-476">En **nombre de usuario** escriba el nombre de inicio de sesión del Administrador de servidor.</span><span class="sxs-lookup"><span data-stu-id="52e8f-476">In **User name** type your server administrator login name.</span></span>
   - <span data-ttu-id="52e8f-477">En **contraseña** escriba la contraseña de inicio de sesión del Administrador de servidor.</span><span class="sxs-lookup"><span data-stu-id="52e8f-477">In **Password** type your server administrator login password.</span></span>
   - <span data-ttu-id="52e8f-478">Escriba un nuevo nombre de base de datos, por ejemplo: *MVC4SampleDB*.</span><span class="sxs-lookup"><span data-stu-id="52e8f-478">Type a new database name, for example: *MVC4SampleDB*.</span></span>

     <span data-ttu-id="52e8f-479">![Configuración de la cadena de conexión de destino](build-restful-apis-with-aspnet-web-api/_static/image55.png "configuración de la cadena de conexión de destino")</span><span class="sxs-lookup"><span data-stu-id="52e8f-479">![Configuring destination connection string](build-restful-apis-with-aspnet-web-api/_static/image55.png "Configuring destination connection string")</span></span>

     <span data-ttu-id="52e8f-480">*Configuración de la cadena de conexión de destino*</span><span class="sxs-lookup"><span data-stu-id="52e8f-480">*Configuring destination connection string*</span></span>
6. <span data-ttu-id="52e8f-481">A continuación, haga clic en **Aceptar**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-481">Then click **OK**.</span></span> <span data-ttu-id="52e8f-482">Cuando se le pedirá que cree la base de datos, haga clic en **Sí**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-482">When prompted to create the database click **Yes**.</span></span>

    <span data-ttu-id="52e8f-483">![Creación de la base de datos](build-restful-apis-with-aspnet-web-api/_static/image56.png "creación de la cadena de la base de datos")</span><span class="sxs-lookup"><span data-stu-id="52e8f-483">![Creating the database](build-restful-apis-with-aspnet-web-api/_static/image56.png "Creating the database string")</span></span>

    <span data-ttu-id="52e8f-484">*Creación de la base de datos*</span><span class="sxs-lookup"><span data-stu-id="52e8f-484">*Creating the database*</span></span>
7. <span data-ttu-id="52e8f-485">La cadena de conexión que se usará para conectarse a la base de datos de SQL en Windows Azure se muestra en el cuadro de texto de conexión predeterminado.</span><span class="sxs-lookup"><span data-stu-id="52e8f-485">The connection string you will use to connect to SQL Database in Windows Azure is shown within Default Connection textbox.</span></span> <span data-ttu-id="52e8f-486">Después, haga clic en **Siguiente**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-486">Then click **Next**.</span></span>

    <span data-ttu-id="52e8f-487">![Cadena de conexión que apunte a la base de datos SQL](build-restful-apis-with-aspnet-web-api/_static/image57.png "cadena de conexión que apunte a la base de datos SQL")</span><span class="sxs-lookup"><span data-stu-id="52e8f-487">![Connection string pointing to SQL Database](build-restful-apis-with-aspnet-web-api/_static/image57.png "Connection string pointing to SQL Database")</span></span>

    <span data-ttu-id="52e8f-488">*Cadena de conexión que apunte a la base de datos SQL*</span><span class="sxs-lookup"><span data-stu-id="52e8f-488">*Connection string pointing to SQL Database*</span></span>
8. <span data-ttu-id="52e8f-489">En el **Preview** página, haga clic en **publicar**.</span><span class="sxs-lookup"><span data-stu-id="52e8f-489">In the **Preview** page, click **Publish**.</span></span>

    <span data-ttu-id="52e8f-490">![Publicar la aplicación web](build-restful-apis-with-aspnet-web-api/_static/image58.png "publicar la aplicación web")</span><span class="sxs-lookup"><span data-stu-id="52e8f-490">![Publishing the web application](build-restful-apis-with-aspnet-web-api/_static/image58.png "Publishing the web application")</span></span>

    <span data-ttu-id="52e8f-491">*Publicar la aplicación web*</span><span class="sxs-lookup"><span data-stu-id="52e8f-491">*Publishing the web application*</span></span>
9. <span data-ttu-id="52e8f-492">Una vez que finalice el proceso de publicación, el explorador predeterminado abrirá el sitio web publicado.</span><span class="sxs-lookup"><span data-stu-id="52e8f-492">Once the publishing process finishes, your default browser will open the published web site.</span></span>

    <span data-ttu-id="52e8f-493">![Publica la aplicación en Windows Azure](build-restful-apis-with-aspnet-web-api/_static/image59.png "publica la aplicación en Windows Azure")</span><span class="sxs-lookup"><span data-stu-id="52e8f-493">![Application published to Windows Azure](build-restful-apis-with-aspnet-web-api/_static/image59.png "Application published to Windows Azure")</span></span>

    <span data-ttu-id="52e8f-494">*Aplicación publicada en Azure*</span><span class="sxs-lookup"><span data-stu-id="52e8f-494">*Application published to Azure*</span></span>

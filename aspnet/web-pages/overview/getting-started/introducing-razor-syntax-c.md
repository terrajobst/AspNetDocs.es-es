...
<span data-ttu-id="8e8c8-149">Para insertar comillas dobles, use un literal de cadena textual y repita las comillas:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-149">To embed double quotation marks, use a verbatim string literal and repeat the quotation marks:</span></span>
...
## <a name="a-simple-code-example"></a><span data-ttu-id="8e8c8-181">Un ejemplo de código simple</span><span class="sxs-lookup"><span data-stu-id="8e8c8-181">A Simple Code Example</span></span>
...
## <a name="basic-syntax"></a><span data-ttu-id="8e8c8-238">Sintaxis básica</span><span class="sxs-lookup"><span data-stu-id="8e8c8-238">Basic Syntax</span></span>
...
## <a name="variables"></a><span data-ttu-id="8e8c8-290">Variables</span><span class="sxs-lookup"><span data-stu-id="8e8c8-290">Variables</span></span>
...
<span data-ttu-id="8e8c8-302">Si combina los ejemplos anteriores en una página, verá que se muestra en un explorador:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-302">If you combine the previous examples in a page, you see this displayed in a browser:</span></span>
...
<span data-ttu-id="8e8c8-317">En la tabla siguiente se enumeran algunos métodos de conversión y prueba comunes para las variables.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-317">The following table lists some common conversion and test methods for variables.</span></span>
...
### <a name="testing-conditions"></a><span data-ttu-id="8e8c8-403">Condiciones de prueba</span><span class="sxs-lookup"><span data-stu-id="8e8c8-403">Testing Conditions</span></span>
...
<span data-ttu-id="8e8c8-410">En este ejemplo, si la primera condición del bloque If no es true, se comprueba la condición `else if`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="8e8c8-411">Si se cumple esta condición, se ejecutan las instrucciones del bloque `else if`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="8e8c8-412">Si no se cumple ninguna de las condiciones, se ejecutan las instrucciones del bloque `else`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="8e8c8-413">Puede agregar cualquier número de bloques else if y, a continuación, cerrar con un bloque `else` como el &quot;todo lo demás&quot; condición.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="8e8c8-414">Para probar un gran número de condiciones, use un bloque `switch`:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="8e8c8-415">El valor que se va a probar se encuentra entre paréntesis (en el ejemplo, la variable `weekday`).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="8e8c8-416">Cada prueba individual utiliza una instrucción `case` que termina con dos puntos (:).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="8e8c8-417">Si el valor de una instrucción `case` coincide con el valor de prueba, se ejecuta el código de ese bloque de casos.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="8e8c8-418">Cierre cada instrucción case con una instrucción `break`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="8e8c8-419">(Si olvida incluir break en cada bloque `case`, el código de la siguiente instrucción `case` se ejecutará también). Un bloque `switch` suele tener una instrucción `default` como el último caso para un &quot;todo lo demás&quot; opción que se ejecuta si no se cumple ninguno de los otros casos.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="8e8c8-420">El resultado de los dos últimos bloques condicionales mostrados en un explorador:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor: Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="8e8c8-422">Código de bucle</span><span class="sxs-lookup"><span data-stu-id="8e8c8-422">Looping Code</span></span>

<span data-ttu-id="8e8c8-423">A menudo es necesario ejecutar las mismas instrucciones varias veces.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="8e8c8-424">Para ello, puede crear un bucle.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-424">You do this by looping.</span></span> <span data-ttu-id="8e8c8-425">Por ejemplo, a menudo se ejecutan las mismas instrucciones para cada elemento en una colección de datos.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="8e8c8-426">Si sabe exactamente cuántas veces desea crear un bucle, puede usar un bucle `for`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="8e8c8-427">Este tipo de bucle es especialmente útil para la acumulación o el recuento:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="8e8c8-428">El bucle comienza con la palabra clave `for`, seguido de tres instrucciones entre paréntesis, cada una termina con un punto y coma.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="8e8c8-429">Dentro de los paréntesis, la primera instrucción (`var i=10;`) crea un contador y lo inicializa en 10.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="8e8c8-430">No tiene que asignar un nombre al contador &#8212; `i` puede usar cualquier variable.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="8e8c8-431">Cuando se ejecuta el bucle `for`, el contador se incrementa automáticamente.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="8e8c8-432">La segunda instrucción (`i < 21;`) establece la condición de cuánto desea contar.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="8e8c8-433">En este caso, desea que pase a un máximo de 20 (es decir, continúe mientras el contador sea inferior a 21).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="8e8c8-434">La tercera instrucción (`i++`) utiliza un operador de incremento, que simplemente especifica que el contador debería tener 1 agregado cada vez que se ejecuta el bucle.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="8e8c8-435">Dentro de las llaves está el código que se ejecutará para cada iteración del bucle.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="8e8c8-436">El marcado crea un nuevo párrafo (elemento`<p>`) cada vez y agrega una línea a la salida, mostrando el valor de `i` (el contador).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="8e8c8-437">Al ejecutar esta página, en el ejemplo se crean 11 líneas que muestran la salida, con el texto en cada línea que indica el número de elemento.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor: Img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="8e8c8-439">Si está trabajando con una colección o una matriz, a menudo utiliza un bucle `foreach`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="8e8c8-440">Una colección es un grupo de objetos similares y el bucle `foreach` le permite llevar a cabo una tarea en cada elemento de la colección.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="8e8c8-441">Este tipo de bucle es práctico para las colecciones, porque a diferencia de un bucle `for`, no tiene que incrementar el contador o establecer un límite.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="8e8c8-442">En su lugar, el código de bucle `foreach` simplemente continúa a través de la colección hasta que termina.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="8e8c8-443">Por ejemplo, el código siguiente devuelve los elementos de la colección `Request.ServerVariables`, que es un objeto que contiene información sobre el servidor Web.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="8e8c8-444">Usa un bucle `foreac` h para mostrar el nombre de cada elemento mediante la creación de un nuevo elemento `<li>` en una lista con viñetas HTML.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="8e8c8-445">La palabra clave `foreach` va seguida de paréntesis en el que se declara una variable que representa un único elemento de la colección (en el ejemplo, `var item`), seguido de la palabra clave `in`, seguido de la colección a la que se desea crear un bucle.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="8e8c8-446">En el cuerpo del bucle `foreach`, puede tener acceso al elemento actual mediante la variable que declaró anteriormente.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor: Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="8e8c8-448">Para crear un bucle de uso más general, use la instrucción `while`:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="8e8c8-449">Un bucle `while` comienza con la palabra clave `while`, seguido de paréntesis en el que se especifica cuánto tiempo continúa el bucle (aquí, siempre y cuando `countNum` sea inferior a 50), después el bloque que se va a repetir.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="8e8c8-450">Los bucles suelen incrementar (agregar a) o disminuir (restar de) una variable o un objeto que se usa para el recuento.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="8e8c8-451">En el ejemplo, el operador `+=` agrega 1 a `countNum` cada vez que se ejecuta el bucle.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="8e8c8-452">(Para reducir una variable en un bucle que se recuenta, se usaría el operador de decremento `-=`).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="8e8c8-453">Objetos y colecciones</span><span class="sxs-lookup"><span data-stu-id="8e8c8-453">Objects and Collections</span></span>

<span data-ttu-id="8e8c8-454">Casi todo en un sitio web de ASP.NET es un objeto, incluida la propia página web.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="8e8c8-455">En esta sección se describen algunos objetos importantes con los que trabajará con frecuencia en el código.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="8e8c8-456">Objetos de página</span><span class="sxs-lookup"><span data-stu-id="8e8c8-456">Page Objects</span></span>

<span data-ttu-id="8e8c8-457">El objeto más básico de ASP.NET es la página.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="8e8c8-458">Puede tener acceso directamente a las propiedades del objeto de página sin ningún objeto calificador.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="8e8c8-459">En el código siguiente se obtiene la ruta de acceso del archivo de la página mediante el `Request` objeto de la página:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="8e8c8-460">Para aclarar que está haciendo referencia a las propiedades y los métodos del objeto de página actual, puede usar opcionalmente la palabra clave `this` para representar el objeto de página en el código.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="8e8c8-461">Este es el ejemplo de código anterior, con `this` agregados para representar la página:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="8e8c8-462">Puede utilizar las propiedades del objeto `Page` para obtener una gran cantidad de información, como:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="8e8c8-463">`Request`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-463">`Request`.</span></span> <span data-ttu-id="8e8c8-464">Como ya ha visto, se trata de una colección de información sobre la solicitud actual, incluido el tipo de explorador que realizó la solicitud, la dirección URL de la página, la identidad del usuario, etc.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="8e8c8-465">`Response`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-465">`Response`.</span></span> <span data-ttu-id="8e8c8-466">Se trata de una colección de información sobre la respuesta (página) que se enviará al explorador cuando el código del servidor haya terminado de ejecutarse.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="8e8c8-467">Por ejemplo, puede usar esta propiedad para escribir información en la respuesta.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="8e8c8-468">Objetos de colección (matrices y diccionarios)</span><span class="sxs-lookup"><span data-stu-id="8e8c8-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="8e8c8-469">Una *colección* es un grupo de objetos del mismo tipo, como una colección de objetos `Customer` de una base de datos.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="8e8c8-470">ASP.NET contiene muchas colecciones integradas, como la `Request.Files` colección.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="8e8c8-471">A menudo, trabajará con los datos de las colecciones.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-471">You'll often work with data in collections.</span></span> <span data-ttu-id="8e8c8-472">Dos tipos de colección comunes son la *matriz* y el *Diccionario*.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="8e8c8-473">Una matriz es útil cuando desea almacenar una colección de elementos similares pero no desea crear una variable independiente que contenga cada elemento:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="8e8c8-474">Con las matrices, se declara un tipo de datos específico, como `string`, `int`o `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="8e8c8-475">Para indicar que la variable puede contener una matriz, agregue corchetes a la declaración (por ejemplo, `string[]` o `int[]`).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="8e8c8-476">Puede tener acceso a los elementos de una matriz mediante su posición (índice) o mediante la instrucción `foreach`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="8e8c8-477">Los índices de matriz son de base &#8212; cero, es decir, el primer elemento está en la posición 0, el segundo elemento está en la posición 1, y así sucesivamente.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="8e8c8-478">Puede determinar el número de elementos de una matriz obteniendo su `Length` propiedad.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="8e8c8-479">Para obtener la posición de un elemento específico en la matriz (para buscar en la matriz), use el método `Array.IndexOf`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="8e8c8-480">También puede hacer cosas como invertir el contenido de una matriz (el método `Array.Reverse`) u ordenar el contenido (el método `Array.Sort`).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="8e8c8-481">La salida del código de la matriz de cadenas que se muestra en un explorador:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-481">The output of the string array code displayed in a browser:</span></span>

![Razor: Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="8e8c8-483">Un diccionario es una colección de pares clave-valor, donde se proporciona la clave (o nombre) para establecer o recuperar el valor correspondiente:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="8e8c8-484">Para crear un diccionario, use la palabra clave `new` para indicar que está creando un nuevo objeto de diccionario.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="8e8c8-485">Puede asignar un diccionario a una variable mediante la palabra clave `var`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="8e8c8-486">Los tipos de datos de los elementos del diccionario se indican mediante corchetes angulares (`< >`).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="8e8c8-487">Al final de la declaración, debe agregar un par de paréntesis, porque en realidad es un método que crea un nuevo diccionario.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="8e8c8-488">Para agregar elementos al diccionario, puede llamar al método `Add` de la variable Dictionary (`myScores` en este caso) y, a continuación, especificar una clave y un valor.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="8e8c8-489">Como alternativa, puede usar corchetes para indicar la clave y realizar una asignación simple, como en el ejemplo siguiente:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="8e8c8-490">Para obtener un valor del diccionario, especifique la clave entre corchetes:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="8e8c8-491">Llamar a métodos con parámetros</span><span class="sxs-lookup"><span data-stu-id="8e8c8-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="8e8c8-492">Tal como se ha leído anteriormente en este artículo, los objetos con los que programa pueden tener métodos.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="8e8c8-493">Por ejemplo, un objeto de `Database` podría tener un método `Database.Connect`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="8e8c8-494">Muchos métodos también tienen uno o más parámetros.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="8e8c8-495">Un *parámetro* es un valor que se pasa a un método para que el método pueda completar su tarea.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="8e8c8-496">Por ejemplo, examine una declaración para el método `Request.MapPath`, que toma tres parámetros:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="8e8c8-497">(La línea se ha ajustado para que sea más legible.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="8e8c8-498">Recuerde que puede colocar los saltos de línea prácticamente en cualquier lugar excepto en las cadenas entre comillas).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="8e8c8-499">Este método devuelve la ruta de acceso física en el servidor que corresponde a una ruta de acceso virtual especificada.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="8e8c8-500">Los tres parámetros del método son `virtualPath`, `baseVirtualDir`y `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="8e8c8-501">(Observe que en la declaración, los parámetros se enumeran con los tipos de datos de los datos que aceptarán). Cuando llame a este método, debe proporcionar valores para los tres parámetros.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="8e8c8-502">El sintaxis Razor proporciona dos opciones para pasar parámetros a un método: *parámetros posicionales* y *parámetros con nombre*.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="8e8c8-503">Para llamar a un método mediante parámetros posicionales, se pasan los parámetros en un orden estricto que se especifica en la declaración del método.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="8e8c8-504">(Normalmente se conoce este orden leyendo la documentación del método). Debe seguir el orden y no puede omitir ninguno de los parámetros &#8212; si es necesario, pasa una cadena vacía (`""`) o `null` para un parámetro posicional para el que no tiene un valor.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="8e8c8-505">En el ejemplo siguiente se da por supuesto que tiene una carpeta denominada *scripts* en el sitio Web.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="8e8c8-506">El código llama al método `Request.MapPath` y pasa los valores de los tres parámetros en el orden correcto.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="8e8c8-507">A continuación, se muestra la ruta de acceso asignada resultante.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="8e8c8-508">Cuando un método tiene muchos parámetros, puede mantener el código más legible mediante el uso de parámetros con nombre.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="8e8c8-509">Para llamar a un método con parámetros con nombre, especifique el nombre del parámetro seguido de un signo de dos puntos (:) y, a continuación, el valor.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="8e8c8-510">La ventaja de los parámetros con nombre es que puede pasarlos en cualquier orden que desee.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="8e8c8-511">(Una desventaja es que la llamada al método no es tan compacta).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="8e8c8-512">En el ejemplo siguiente se llama al mismo método que el anterior, pero se usan parámetros con nombre para proporcionar los valores:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="8e8c8-513">Como puede ver, los parámetros se pasan en un orden diferente.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="8e8c8-514">Sin embargo, si ejecuta el ejemplo anterior y este ejemplo, devolverán el mismo valor.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="8e8c8-515">Control de errores</span><span class="sxs-lookup"><span data-stu-id="8e8c8-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="8e8c8-516">Instrucciones try-catch</span><span class="sxs-lookup"><span data-stu-id="8e8c8-516">Try-Catch Statements</span></span>

<span data-ttu-id="8e8c8-517">A menudo tendrá instrucciones en el código que podrían producir errores por motivos ajenos al control.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="8e8c8-518">Por ejemplo:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-518">For example:</span></span>

- <span data-ttu-id="8e8c8-519">Si el código intenta crear o tener acceso a un archivo, puede producirse todo tipo de errores.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="8e8c8-520">Es posible que el archivo que desea no exista, que esté bloqueado, que el código no tenga permisos, etc.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="8e8c8-521">Del mismo modo, si el código intenta actualizar registros en una base de datos, puede haber problemas con los permisos, que la conexión a la base de datos se haya quitado, que los datos que se van a guardar no sean válidos, etc.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="8e8c8-522">En términos de programación, estas situaciones se denominan *excepciones*.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="8e8c8-523">Si el código encuentra una excepción, genera (lanza) un mensaje de error que, en el mejor de los usuarios, le molesta:</span><span class="sxs-lookup"><span data-stu-id="8e8c8-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor: Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="8e8c8-525">En situaciones en las que el código pueda encontrar excepciones y para evitar los mensajes de error de este tipo, puede usar `try/catch` instrucciones.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="8e8c8-526">En la instrucción `try`, ejecute el código que está comprobando.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="8e8c8-527">En una o varias instrucciones de `catch`, puede buscar errores específicos (tipos específicos de excepciones) que puedan haberse producido.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="8e8c8-528">Puede incluir tantas instrucciones `catch` como necesite para buscar los errores que espera.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="8e8c8-529">Se recomienda evitar el uso del método `Response.Redirect` en `try/catch` instrucciones, ya que puede producir una excepción en la página.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="8e8c8-530">En el ejemplo siguiente se muestra una página que crea un archivo de texto en la primera solicitud y, a continuación, muestra un botón que permite al usuario abrir el archivo.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="8e8c8-531">En el ejemplo se usa deliberadamente un nombre de archivo incorrecto para que se produzca una excepción.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="8e8c8-532">El código incluye `catch` instrucciones para dos posibles excepciones: `FileNotFoundException`, que se produce si el nombre de archivo es incorrecto y `DirectoryNotFoundException`, que se produce si ASP.NET no puede incluso encontrar la carpeta.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="8e8c8-533">(Puede quitar el comentario de una instrucción en el ejemplo para ver cómo se ejecuta cuando todo funciona correctamente).</span><span class="sxs-lookup"><span data-stu-id="8e8c8-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="8e8c8-534">Si el código no controla la excepción, verá una página de error como la captura de pantalla anterior.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="8e8c8-535">Sin embargo, la sección `try/catch` ayuda a evitar que el usuario vea estos tipos de errores.</span><span class="sxs-lookup"><span data-stu-id="8e8c8-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="8e8c8-536">Recursos adicionales</span><span class="sxs-lookup"><span data-stu-id="8e8c8-536">Additional Resources</span></span>

<span data-ttu-id="8e8c8-537">**Programar con Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="8e8c8-537">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="8e8c8-538">Apéndice: lenguaje de Visual Basic y sintaxis</span><span class="sxs-lookup"><span data-stu-id="8e8c8-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="8e8c8-539">**Documentación de referencia**</span><span class="sxs-lookup"><span data-stu-id="8e8c8-539">**Reference Documentation**</span></span>

[<span data-ttu-id="8e8c8-540">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="8e8c8-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="8e8c8-541">C#Módulo</span><span class="sxs-lookup"><span data-stu-id="8e8c8-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)

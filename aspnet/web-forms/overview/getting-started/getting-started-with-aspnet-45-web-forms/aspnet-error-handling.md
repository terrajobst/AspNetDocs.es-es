...
ms.locfileid: "69995310"
---
# <a name="aspnet-error-handling"></a>Control de errores de ASP.NET

por [Erik Reitan](https://github.com/Erikre)

[Descargar el proyecto de ejemplo deC#Wingtip Toys ()](http://go.microsoft.com/fwlink/?LinkID=389434&clcid=0x409) o [descargar el libro electrónico (pdf)](http://download.microsoft.com/download/0/F/B/0FBFAA46-2BFD-478F-8E56-7BF3C672DF9D/Getting%20Started%20with%20ASP.NET%204.5%20Web%20Forms%20and%20Visual%20Studio%202013.pdf)
...
Como procedimiento recomendado, controle los errores en el nivel de código `Try` en / / `Catch` `Finally` los bloques dentro del código. Intente colocar estos bloques para que el usuario pueda corregir los problemas en el contexto en el que se producen. Si los bloques de control de errores están demasiado lejos de donde se produjo el error, resulta más difícil proporcionar a los usuarios la información necesaria para solucionar el problema.

### <a name="exception-class"></a>Exception (clase)

La clase de excepción es la clase base de la que heredan las excepciones. La mayoría de los objetos de excepción son instancias de alguna clase derivada de la clase de `SystemException` excepción, como `IndexOutOfRangeException` la clase, la `ArgumentNullException` clase o la clase. La clase de excepción tiene propiedades, como la `StackTrace` propiedad, la `InnerException` propiedad y la `Message` propiedad, que proporcionan información específica sobre el error que se ha producido.

### <a name="exception-inheritance-hierarchy"></a>Jerarquía de herencia de excepciones

El motor en tiempo de ejecución tiene un conjunto base de excepciones `SystemException` que derivan de la clase que el Runtime inicia cuando se encuentra una excepción. La mayoría de las clases que heredan de la clase de excepción, `IndexOutOfRangeException` como la clase `ArgumentNullException` y la clase, no implementan miembros adicionales. Por lo tanto, la información más importante para una excepción puede encontrarse en la jerarquía de excepciones, el nombre de la excepción y la información contenida en la excepción.
...
Puede controlar los errores predeterminados y los errores http `customErrors` agregando una sección al archivo *Web. config* . La `customErrors` sección le permite especificar una página predeterminada a la que se redirigirán los usuarios cuando se produzca un error. También permite especificar páginas individuales para errores de código de estado específicos.
...
Sin embargo, puede interceptar los errores que se produzcan en cualquier parte de la `Application_Error` aplicación agregando código al controlador en el archivo *global. asax* .
...
En este ejemplo de código se muestra un controlador para el evento de error en una página web de ASP.NET. Este controlador detecta todas las excepciones que todavía no se han controlado `try` dentro / `catch` de los bloques de la página.

[!code-csharp[Main](aspnet-error-handling/samples/sample3.cs)]

Después de controlar un error, debe borrarlo llamando `ClearError` al método del objeto de servidor (`HttpServerUtility` clase); de lo contrario, verá un error que se ha producido previamente.
...
En el ejemplo de código siguiente se muestra una manera `try` común de utilizar / / `catch` `finally` para controlar los errores.

[!code-csharp[Main](aspnet-error-handling/samples/sample4.cs)]

En el código anterior, el bloque try contiene el código que debe protegerse contra una posible excepción. El bloque se ejecuta hasta que se produce una excepción o hasta que el bloque se completa correctamente. Si se produce `FileNotFoundException` una excepción o `IOException` una excepción, la ejecución se transfiere a una página diferente. A continuación, se ejecuta el código contenido en el bloque Finally, tanto si se ha producido un error como si no.
...
2. Seleccione el grupo plantillas de **código** **Visual C#**   - &gt; de la izquierda. A continuación, seleccione **clase**en la lista central y asígnele el nombre **ExceptionUtility.CS**.
3. Haga clic en **Agregar**. Se muestra el nuevo archivo de clase.
4. Reemplace el código existente por el siguiente:  

    [!code-csharp[Main](aspnet-error-handling/samples/sample5.cs)]

Cuando se produce una excepción, la excepción se puede escribir en un archivo de registro de excepción `LogException` mediante una llamada al método. Este método toma dos parámetros, el objeto de excepción y una cadena que contiene los detalles sobre el origen de la excepción. El registro de excepciones se escribe en el archivo *ErrorLog. txt* de la carpeta de datos de la *\_aplicación* .
...
1. Haga clic con el botón derecho en el nombre del proyecto (**Wingtip Toys**) en **Explorador de soluciones** y seleccione **Agregar**  - &gt; **nuevo elemento**.   
   Se abrirá el cuadro de diálogo **Agregar nuevo elemento**.
2. Seleccione el grupo plantillas **Web** de **Visual C#**   - &gt; de la izquierda. En la lista central, seleccione **Web Forms con página maestra**y asígnele el nombre **ErrorPage. aspx**.
...
Al agregar una `customErrors` sección al archivo *Web. config* , puede controlar rápidamente los errores sencillos que se producen en toda la aplicación. También puede especificar cómo controlar los errores en función de su valor de código de estado, como 404-archivo no encontrado.
...
2. En el `Page_Load` controlador, agregue código para que el controlador aparezca como sigue:   

    [!code-csharp[Main](aspnet-error-handling/samples/sample9.cs?highlight=3-4)]

Es posible crear distintos tipos de excepciones. En el código anterior, está creando una `InvalidOperationException` cuando se carga la página *default. aspx* .
...
Como puede ver en los detalles del error, la excepción se ha capturado en `customError` la sección del archivo *Web. config* .

### <a name="adding-application-level-error-handling"></a>Agregar el control de errores en el nivel de aplicación

En lugar de interceptar la excepción `customErrors` mediante la sección del archivo *Web. config* , donde obtendrá poca información acerca de la excepción, puede interceptar el error en el nivel de aplicación y recuperar los detalles del error.

1. En **Explorador de soluciones**, busque y abra el archivo *global.asax.CS* .
2. Agregue un controlador de **errores de aplicación\_** para que aparezca de la manera siguiente:   

    [!code-csharp[Main](aspnet-error-handling/samples/sample10.cs)]

Cuando se produce un error en la aplicación, `Application_Error` se llama al controlador. En este controlador, se recupera y revisa la última excepción. Si no se controló la excepción y la excepción contiene detalles de la excepción interna (es decir `InnerException` , is not null), la aplicación transfiere la ejecución a la página de error donde se muestran los detalles de la excepción.
...
1. Presione **Ctrl + F5** para ejecutar la aplicación de ejemplo Wingtip Toys.  
 La aplicación produce la `InvalidOperationException` excepción.
2. Revise el de *ErrorPage. aspx* mostrado en el explorador. 
...
Puede Agregar un control de errores en el nivel de página a una página, ya `ErrorPage` sea mediante la `@Page` adición de un atributo a la Directiva de la `Page_Error` página o mediante la adición de un controlador de eventos al código subyacente de una página. En esta sección, agregará un `Page_Error` controlador de eventos que transferirá la ejecución a la página *ErrorPage. aspx* .

1. En **Explorador de soluciones**, busque y abra el archivo *default.aspx.CS* .
2. Agregue un `Page_Error` controlador para que el código subyacente aparezca de la manera siguiente:   

    [!code-csharp[Main](aspnet-error-handling/samples/sample11.cs?highlight=18-30)]

Cuando se produce un error en la página, `Page_Error` se llama al controlador de eventos. En este controlador, se recupera y revisa la última excepción. Si se `InvalidOperationException` produce una excepción `Page_Error` , el controlador de eventos transfiere la ejecución a la página de error donde se muestran los detalles de la excepción.
...
2. En el `Page_Load` controlador, quite el código que produce la excepción para que el controlador aparezca de la manera siguiente:   
...
2. Actualice el `HttpCall` método para que el código aparezca como sigue:   

    [!code-csharp[Main](aspnet-error-handling/samples/sample13.cs?highlight=20,22-23)]

El código anterior llama al `LogException` método contenido en la `ExceptionUtility` clase. Anteriormente en este tutorial, ha agregado el archivo de clase *ExceptionUtility.CS* a la carpeta *lógica* . El método `LogException` toma dos parámetros. El primer parámetro es el objeto de excepción. El segundo parámetro es una cadena que se usa para reconocer el origen del error.
...
1. En **Explorador de soluciones**, busque y abra el archivo *ErrorLog. txt* en la carpeta de datos de la *aplicación\_* .   
...
- Graham Mendick ([@grahammendick](http://twitter.com/grahammendick))  
  Ejemplo de código relacionado con Visual Studio 2012 en MSDN: [Navegación de Wingtip Toys](https://code.msdn.microsoft.com/Navigation-Wingtip-Toys-5f0daba2)
- James Chaney ([jchaney@agvance.net](mailto:jchaney@agvance.net))  
  Ejemplo de código relacionado con Visual Studio 2012 en MSDN: [ASP.NET 4,5 tutorial de formularios Web Forms en Visual Basic](https://code.msdn.microsoft.com/ASPNET-45-Web-Forms-f37f0f63)
- Andrielle Azevedo-colaborador de la audiencia técnica de @driazevedoMicrosoft (Twitter:)  
  Traducción de Visual Studio 2012: [Iniciando com ASP.NET Web Forms 4,5-parte 1-Introdução e Visão Geral](https://andrielleazevedo.wordpress.com/2013/01/24/iniciando-com-asp-net-web-forms-4-5-introducao-e-visao-geral/)

...
[Vea o descargue el código de ejemplo](https://github.com/dotnet/AspNetDocs/tree/master/aspnet/web-forms/overview/older-versions-getting-started/deploying-web-site-projects/processing-unhandled-exceptions-vb/samples) ([cómo descargarlo](/aspnet/core/tutorials/index#how-to-download-a-sample))
...
**Figura 1**: agregar `Global.asax` a la aplicación Web  
 ([Haga clic para ver la imagen de tamaño completo](processing-unhandled-exceptions-vb/_static/image3.png))

El contenido y la estructura del archivo `Global.asax` creado por Visual Studio difieren ligeramente en función de si se usa un proyecto de aplicación web (WAP) o un proyecto de sitio web (WSP). Con WAP, el `Global.asax` se implementa como dos archivos independientes: `Global.asax` y `Global.asax.vb`. El archivo `Global.asax` no contiene nada excepto una directiva `@Application` que haga referencia al archivo `.vb`; los controladores de eventos de interés se definen en el archivo de `Global.asax.vb`. En el caso de WSPs, solo se crea un único archivo, `Global.asax`, y los controladores de eventos se definen en un bloque de `<script runat="server">`.
...
> [!NOTE]
> Al implementar la aplicación ASP.NET, debe copiar el archivo de `Global.asax` en el entorno de producción. No es necesario copiar el archivo de `Global.asax.vb`, que se crea en WAP, en producción, ya que este código se compila en el ensamblado del proyecto.
...
**Figura 2**: se envía una notificación por correo electrónico al desarrollador cada vez que hay una excepción no controlada  
 ([Haga clic para ver la imagen de tamaño completo](processing-unhandled-exceptions-vb/_static/image6.png))
...
**Figura 3**: la notificación de correo electrónico incluye los detalles de la excepción YSOD como datos adjuntos  
 ([Haga clic para ver la imagen de tamaño completo](processing-unhandled-exceptions-vb/_static/image9.png))
...
**Figura 4**: cuando se produce un error, el explorador se redirige a la dirección URL de la página de errores personalizada  
 ([Haga clic para ver la imagen de tamaño completo](processing-unhandled-exceptions-vb/_static/image12.png))

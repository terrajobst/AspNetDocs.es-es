...
title: Actualización por lotesC#() | Microsoft Docs
...
ms.locfileid: "78476407"
---
# <a name="batch-updating-c"></a>Actualización por lotes (C#)

por [Scott Mitchell](https://twitter.com/ScottOnWriting)

[Descargar código](https://download.microsoft.com/download/3/9/f/39f92b37-e92e-4ab3-909e-b4ef23d01aa3/ASPNET_Data_Tutorial_64_CS.zip) o [Descargar PDF](batch-updating-cs/_static/datatutorial64cs1.pdf)
...
En el [tutorial anterior](wrapping-database-modifications-within-a-transaction-cs.md) , vimos cómo extender el nivel de acceso a datos para agregar compatibilidad con transacciones de bases de datos. Las transacciones de base de datos garantizan que una serie de instrucciones de modificación de datos se tratará como una operación atómica, lo que garantiza que se producirá un error en todas las modificaciones o todas se realizarán correctamente. Gracias a esta funcionalidad DAL de bajo nivel, ya se ha preparado para activar la atención de la creación de interfaces de modificación de datos por lotes.
...
[![configurar ObjectDataSource para usar la clase ProductsBLL](batch-updating-cs/_static/image2.gif)](batch-updating-cs/_static/image3.png)

**Figura 2**: configuración de ObjectDataSource para usar la clase `ProductsBLL` ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image4.png))

[![recuperar los datos del producto mediante el método GetProducts](batch-updating-cs/_static/image3.gif)](batch-updating-cs/_static/image5.png)

**Figura 3**: recuperación de los datos del producto con el método `GetProducts` ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image6.png))
...
[![establecer las listas desplegables de las pestañas actualizar, insertar y eliminar en (ninguna)](batch-updating-cs/_static/image4.gif)](batch-updating-cs/_static/image7.png)

**Figura 4**: establecer las listas desplegables de las pestañas actualizar, insertar y eliminar en (ninguna) ([haga clic para ver la imagen a tamaño completo](batch-updating-cs/_static/image8.png))
...
![Convertir los BoundFields existentes y CheckBoxField en TemplateField](batch-updating-cs/_static/image5.gif)

**Figura 5**: conversión de BoundFields y CheckBoxField existentes en TemplateField
...
[![el TemplateField de ProductName incluye ahora un TextBox y un RequiredFieldValidator](batch-updating-cs/_static/image6.gif)](batch-updating-cs/_static/image9.png)

**Figura 6**: el `ProductName` TemplateField ahora incluye un cuadro de texto y un RequiredFieldValidator ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image10.png))
...
[![agregar un CompareValidator para asegurarse de que el precio introducido es un valor de moneda no negativo](batch-updating-cs/_static/image7.gif)](batch-updating-cs/_static/image11.png)

**Figura 7**: agregar un CompareValidator para asegurarse de que el precio introducido es un valor de moneda no negativo ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image12.png))
...
[![configurar ObjectDataSource para usar la clase CategoriesBLL](batch-updating-cs/_static/image8.gif)](batch-updating-cs/_static/image13.png)

**Figura 8**: configuración de ObjectDataSource para usar la clase `CategoriesBLL` ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image14.png))

[![recuperar los datos de la categoría mediante el método GetCategories](batch-updating-cs/_static/image9.gif)](batch-updating-cs/_static/image15.png)

**Figura 9**: recuperación de los datos de la categoría mediante el método `GetCategories` ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image16.png))
...
[![establecer las listas desplegables de las pestañas actualizar y eliminar en (ninguna)](batch-updating-cs/_static/image10.gif)](batch-updating-cs/_static/image17.png)

**Figura 10**: establecer las listas desplegables de las pestañas actualizar y eliminar en (ninguna) ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image18.png))
...
[![enlazar el DropDownList a CategoriesDataSource](batch-updating-cs/_static/image11.gif)](batch-updating-cs/_static/image19.png)

**Figura 11**: enlace de DropDownList al `CategoriesDataSource` ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image20.png))
...
![Enlazar el valor de Product s CategoryID a la propiedad de DropDownList s SelectedValue](batch-updating-cs/_static/image12.gif)
...
**Figura 13**: cada fila de GridView es editable ([haga clic para ver la imagen de tamaño completo](batch-updating-cs/_static/image22.png))
...
![Centrar la casilla descontinuada](batch-updating-cs/_static/image15.gif)
...
El algoritmo de actualización por lotes utilizado para este tutorial actualiza cada fila del `ProductsDataTable` que corresponde a una fila de GridView, independientemente de si se ha cambiado la información del producto. Aunque estas actualizaciones ciegas no suelen ser un problema de rendimiento, pueden conducir a registros superfluos si se vuelven a auditar los cambios en la tabla de base de datos. En el tutorial [realización de actualizaciones por lotes](../editing-and-deleting-data-through-the-datalist/performing-batch-updates-cs.md) se ha explorado una interfaz de actualización por lotes con la lista de datos y se ha agregado código que solo actualizaría los registros que el usuario modificó realmente. No dude en usar las técnicas para [realizar actualizaciones por lotes](../editing-and-deleting-data-through-the-datalist/performing-batch-updates-cs.md) para actualizar el código de este tutorial, si lo desea.
...
El código utilizado en `BatchUpdate` es similar al que se usa en los métodos de `UpdateProduct` de BLL, la diferencia principal es que en los métodos `UpdateProduct` solo se recupera de la arquitectura una única instancia de `ProductRow`. El código que asigna las propiedades del `ProductRow` es el mismo entre los métodos `UpdateProducts` y el código dentro del bucle `foreach` en `BatchUpdate`, como es el patrón global.
...
En primer lugar, se realiza una llamada a `BatchUpdate`. A continuación, se usa el `ClientScript property` para insertar JavaScript que mostrará un cuadro de mensajes que indica que los productos se han actualizado.
